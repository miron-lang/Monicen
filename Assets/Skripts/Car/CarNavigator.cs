...
† † void Update()
    ...
    // CarWaypointNafigator включ€ет этот режим только после разрешени€ контроллера
    public void SetIntersectionDriving(bool isDrivingInside)
    {
        ignoreObstaclesInsideIntersection = isDrivingInside; // «апоменаем, находица ли бибика внутри перекрЄстка

        if (isDrivingInside)
        {
            // —тарый сохранЄнный BoxCast не должен остановить бибику после въезда
            cachedObstacleDetected = false; // ”дал€ем сохранЄнное преп€тсие, замеченное до въезда
            cachedObstacleSpeed = maxMovingSped; // ¬озращ€ем разрешЄнную скорость к максималной
        }
        else
        {
            // ѕосле выезда сразу ворощ€ем проверку преп€тствий
            nextObstacleChachTime = Time.time; // ѕросим Update выполнить BoxCast на ближаещем кадре
        }
    }
